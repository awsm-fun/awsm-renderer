...
// Custom-WGSL (dynamic) material: one registered material with a
// `tint: vec3<f32>` uniform, assigned to two spheres — the left reads the
// SHARED default (blue, set_material_uniform), the right carries a
// PER-INSTANCE override (orange, set_node_material_uniform). Correct =
// both spheres shaded by the custom lambert fragment with different tints;
// live uniform edits re-shade without recompiling.
//
// Probed shapes worth keeping:
// - set_custom_material_layout {id, uniforms:[{name, ty}], textures:[],
//   buffers:[]} — `ty` is a WGSL TYPE STRING ("vec3<f32>", "f32", ...);
//   a friendly name like "color3" is accepted silently and falls back to
//   f32 (register then fails with confusing naga compose errors).
// - register_material returns ok immediately; compile status arrives via
//   the `material_diagnostics` query {material} — ALWAYS check
//   {ok:true, registered:true} after registering.
// - set_material_uniform {material, name, value} accepts the TEXT form
//   ("0.2, 0.4, 0.9"); set_node_material_uniform {node, name, value}
//   requires the tagged form {kind:"vec3", value:[..]}.
...
async () => {
  ...
  const wgsl = `let n = normalize(input.world_normal);
...
return OpaqueShadingOutput(input.material.tint * diff, 1.0);`;
  ...
  for (let i = 0; i < 2; i++) {
    ...
    await d({ cmd: 'rename', id: n, name: i === 0 ? 'shared-tint' : 'override-tint' });
  ...
  await d({ cmd: 'set_material_uniform', material: cmat, name: 'tint', value: '0.2, 0.4, 0.9' });
  await d({ cmd: 'set_node_material_uniform', node: ID(0x11), name: 'tint', value: { kind: 'vec3', value: [1, 0.5, 0.1] } });
  await d({ cmd: 'set_camera_orbit', yaw: 0.3, pitch: 0.35, radius: 8, look_at: [0, 0.9, 0] });
