...
- **`Displace{expr}` evaluator**: a small CPU expression evaluator over
  `(x,y,z,nx,ny,nz,u,v,i)` (e.g. evalexpr-style). Then it's per-vertex along the
  normal. Add a native test.
...
## Introspection queries (LLM perceive→act) — ⬜ NOT STARTED
`get_mesh_stats` (bbox/vert/tri/centroid/volume/surface-area/watertight) +
`get_mesh_cross_section` (radius profile along an axis). Pure functions over
`MeshData` → put the math in `meshgen` (native tests) + `EditorQuery` + MCP.

## Phases 4–6 — ⬜ NOT STARTED
- **4** raw per-vertex editing: `CollapseMeshStack`, `SetVertexPositions`,
  `SoftTransformVertices` (server-computed falloff), transient `SetVertexSelection`
  + predicate selection (`select_vertices_where`). Sparse undo (touched verts only).
