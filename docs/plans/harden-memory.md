...
| P3 | Render-worker crash | mt:dev | ✅ (won't-recover, by decision) | **Decided (David, 2026-06-24): a render-worker process death (Rust panic→`abort`, browser OOM-kill) is catastrophic — do NOT recover.** A killed wasm thread can't run destructors, so respawning it orphans its shared-memory allocations (measured ~215 MB/respawn, unbounded); and isolating per-group memory to fix that would forfeit the flexible single-shared-memory model (any thread feeding renderer arenas). A panic is unrecoverable by design anyway. The respawn machinery (watchdog/fresh-canvas/epoch) was prototyped, verified, then **removed**. The recoverable failure — **GPU device loss** — is handled in-place + leak-free by **P2**. |
