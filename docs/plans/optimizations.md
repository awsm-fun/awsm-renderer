...
Optimization work shipped on this branch is archived in §15. The
items below are what's still open after the current session.

### 16.7 / 16.8 — Geometry pass `drawIndirect` rewire

**Status:** `infrastructure landed (commit 06bfc5f); geometry-pass
draw-loop rewire deferred to a dedicated session`. Gated behind
`features.gpu_culling`; the always-on cost it would erase is
already opt-in via §16.F, so the deferral does not regress the
default-library path.

Phase 1 (cull pass) + Phase 2 infra (compaction compute + the
`IndirectDrawArgs` storage buffer) shipped in commits `2034023`
and `06bfc5f`. The compaction populates `IndirectDrawArgs.instance_count`
each frame but the geometry pass doesn't consume it yet — the
existing per-mesh `draw_indexed` loop in
[`meshes/mesh.rs::push_geometry_pass_commands`](../../crates/renderer/src/meshes/mesh.rs)
still records each draw CPU-side.

To wire `drawIndirect` the load-bearing change is in the geometry
vertex shader's per-mesh meta lookup. Today it's a uniform with a
...
The full surface area (the reason this didn't land in the same
session as §16.F / §16.4.C / §16.G):

  - CPU-side population of `IndirectDrawArgs{index_count,
    first_index, base_vertex, first_instance}` per mesh slot on
    insert / update — the args buffer is currently uninitialised
  ...
  - CPU-side population of `occlusion_instances[i].mesh_meta_offset`
    ...
    (see `render.rs` cull-instance pack block), which makes the
  ...
  - A pipeline-cache invalidation surface check: the bind-group
    layout flip (uniform-with-dynamic-offset → read-only-storage)
    ripples through `PipelineLayouts::get_key` and invalidates
    any cached geometry render pipelines built against the
    dynamic-offset shape (fine on a fresh boot; a runtime layout
    swap during the same renderer instance would miss).
  - The matching binding-shape change in the *transparent* pass
    ...
    helpers reference the same symbol. Either keep transparent
    on the old uniform binding (separate WGSL fork) or migrate
    both in lockstep.
...
- The CPU populates `IndirectDrawArgs[mesh].first_instance =
  mesh_meta_index`; the GPU compaction continues to set
  `instance_count`.

*Acceptance:*

- Visual parity with the per-mesh `draw_indexed` path under
  `features.gpu_culling = true`.
- Visual parity with the legacy path under
  `features.gpu_culling = false` (the legacy path stays alive).
- A `tracing` span around the geometry pass shows CPU command
  recording time dropped.
- `tuning-10k-meshes` measures a frame-time win vs the §15
  10k-mesh row's both-on baseline (2.029 ms `Render` mean) once
  the GPU-driven cull actually drops invisible draws.

### 16.T deferred-to-authoring profiling rows

Three §15 rows are recorded as `defer — needs authentic content`:

- **T3** Importance-tier cutoff tuning — current tuning scenes are
  uniform-intensity so the histogram is degenerate. Pick up when a
  real game-side scene with varied light intensities is available.
- **T4** Cheap-material pixel threshold — depends on the GPU
  coverage compute pass (deferred per §15 row 6.2 footnote).
  Without a populated `MeshCoverage` table every mesh is
  zero-coverage and the threshold is moot.
- **T6** Oversized-mesh threshold — needs a
  `#[cfg(debug_assertions)] #[wasm_bindgen]` getter exposing
  `oversized_meshes().len()` + `last_max_bucket`, then a
  `tuning-open-world` re-run. Easy to land alongside the next
  authoring sweep; not load-bearing for the current synthetic
  scenes.
