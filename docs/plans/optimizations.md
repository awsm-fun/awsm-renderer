...
| 2.2  | per-frame cascade       | throttled                   | Landed. Directional cascades migrated off the packed `shadow_atlas` onto a 2D-array depth texture (`cascade_array_texture`), one layer per cascade with per-layer render-attachment views. Per-light `params.resolution` clamps to the config's `cascade_resolution`; smaller cascades fill the top-left sub-rect of their layer. New descriptor kind = 3 (cascade-array PCF); EVSM moment-write reads from a layer of the array instead of the atlas. `LightShadowView::cascade_layer` carries the assignment; throttle invalidates on layer reassignment. `ShadowsConfig` gained `cascade_resolution` (default 2048) + `cascade_array_max_layers` (default 16). |
| 2.3  | evsm every cascade      | evsm only for rendered      | Landed. `EvsmDispatchEntry::should_render` flag set during throttle reconciliation in `Shadows::write_gpu`; the EVSM dispatch loop in `render_pass::dispatch_evsm` skips entries whose flag is false, dropping the moment-write + 2 blur passes for cascades whose layer wasn't refreshed this frame. With `far_cascade_update_rate = Every4Frames` and EVSM on the far cascade, EVSM dispatches scale with rendered-cascade count instead of configured-cascade count. |
...
| 4.3  | widened-Soft cube PCSS  | true cube PCSS              | Landed. Cube pool exposes a second view (`cube_2d_array_view`, `texture_depth_2d_array`) used by PCSS for raw `textureLoad` blocker reads. `sample_shadow_cube` branches on hardness: `Hard` (1 tap), `Soft` (16-tap rotated Poisson, 15 cm disc), `Pcss` (16-tap blocker search on the 2D-array view → variable-kernel PCF on the cube sampler). Penumbra width per the standard `(z_recv − z_blocker_avg) / z_blocker_avg` mapped to a world-space disc radius clamped to `[10 cm, 1 m]`. Face-projection math is inlined at the call site (forward-declared helper tripped a Dawn validation error). |
...
| 6.1  | N × screen-wide compute | N × tile-bucket compute     | Landed. (A) Opaque compute pass specialised per `MaterialShaderId` (PBR / Unlit / Toon) — `cache_key.rs` gained `shader_id`, `template.rs` switched the runtime `if` to a `{% match shader_id %}` template choice, `pipeline.rs` caches per `(msaa, mipmaps, shader_id)`. (B) `material_classify` pass scans the visibility buffer per 8×8 tile, atomic-or's a 4-bit shader_id mask, then per workgroup appends tile coords to `bucket[shader_id]` + bumps `indirect_args[shader_id].x`. Each opaque variant consumes its bucket via `dispatch_workgroups_indirect`. Skybox pixels (`triangle_index == U32_MAX`) are PBR-shaded by convention; non-PBR pipelines early-return on skybox so a mixed-material tile doesn't double-write skybox. |
| 6.2  | all skinning every frame | only-visible skinning      | Landed end-to-end. CPU consumer: `MeshCoverage` table + `Meshes::skin_all_consumers_zero_coverage`. GPU producer: new `render_passes/coverage/` compute pass dispatches one thread per pixel after the geometry pass, atomic-adding 1 into `mesh_pixel_counts[material_mesh_meta_offset / 256]`. The renderer zeros the counts buffer per-frame, copies it into a `MAP_READ` readback buffer, and `spawn_local`s a `mapAsync` task that resolves on a future frame and routes the bytes through `MeshCoverage::ingest`. Single-buffered readback (an `inflight` flag short-circuits the next kick while a prior `mapAsync` is still pending — under high mapping latency we drop a frame of coverage rather than ring the buffer). MSAA path forks the bind-group layout via `multisampled` on the cache key + a `texture_multisampled_2d<u32>` binding; sample-index 0 is enough since the consumer signal is "did this mesh contribute any pixels". Plan §8.2. |
| 6.3  | full material everywhere | cheap-mat at low coverage  | Landed end-to-end. `Mesh::cheap_material_key` + `cheap_material_pixel_threshold: Option<u32>`; `Mesh::effective_material_key`; `collect_renderables` classifies transparency by the effective key. Per-mesh threshold falls back to `AwsmRenderer::default_cheap_material_pixel_threshold` (plan §15 row T4) when `None`. GPU coverage producer (`render_passes/coverage/`, plan §8.2) feeds the `MeshCoverage` table the consumer reads from. |
| 6.4  | n/a                     | decals as material class    | Landed. Decima/D3-style projection decals: `decals/{api,data,gpu}.rs` carries the slotmap + per-decal GPU buffer + `AwsmRenderer::{insert,update,remove}_decal` API. `NodeKind::Decal` in `awsm-scene-schema` with `DecalConfig { texture, alpha, blend_mode }`. The `material_decal` compute pass reconstructs world-space position from depth and alpha-blends decals whose inverse-transformed point lies inside the unit cube. Per-tile classify pass (§16.4.C) projects each decal's world AABB to screen + appends to overlapping 8×8 tile buckets (capacity 32, overflow drops). MSAA composite (`composite.rs`) blits a single-sample `decal_color` storage texture onto the multisampled transparent target. HZB occlusion gate active when `features.gpu_culling && features.decals` (this session — see row 16.4.C above). |
...
| 7.1  | n/a                     | hzb build cost              | Landed. `render_passes/hzb/` owns the `r32float` mip-chain texture (sized to viewport via `HzbRenderPass::ensure_size`, per-mip single-level storage views). Two compute shaders: `seed.wgsl` copies depth → mip 0; `reduce.wgsl` max-reduces a 2×2 of mip N-1 into mip N. One dispatch per mip transition with the bounds check inside the shader. Stores **maximum** depth per tile so consumers run the canonical "occluded if closest-screen-depth > HZB max" test. Slotted between `material_decal` and `line`. Gated behind `features.gpu_culling` (§16.F). |
| 7.2  | bvh-only cull           | bvh + hzb cull              | Landed. `OcclusionRenderPass` (commit `2034023`) reads HZB + per-instance AABBs and writes `visible_this_frame[i]`. The geometry pass consumes this via §16.7/§16.8's draw-loop rewire — see the dedicated row below. Gated behind `features.gpu_culling` (§16.F). |
| 7.3  | cpu instance loop       | gpu instance compaction     | Landed. `CompactionRenderPass` (commit `06bfc5f`) reads `visible_this_frame[i]` and atomic-adds `1` into `indirect_args[mesh_meta_idx].instance_count`. The geometry pass consumes the populated args via `drawIndexedIndirect` — see the §16.7/§16.8 row below for the full draw-loop rewire. Gated behind `features.gpu_culling` (§16.F). |
...
| 16.F | always-on infra (HZB, occlusion cull, compaction, decal resources) | opt-in via `RendererFeatures` | **Measured** (Claude Preview MCP, headless Chrome, 120-frame mean) — post-§16.7/§16.8 numbers. Empty-scene `Render` span: 0.524 ms both-on vs 0.395 ms both-off → +0.129 ms always-on overhead. Per-scene (post-rewire): tuning-1k-meshes +0.008 ms (1.645 vs 1.637), tuning-64-lights +0.127 ms (0.790 vs 0.663), tuning-10k-meshes +0.105 ms (1.912 vs 1.807). The 1k-mesh gap collapsed once `drawIndirect` recovered the per-mesh `set_bind_group` cost — net always-on cost is now noise-level on the 1k tier. Gated `HZB` / `Occlusion Cull` / `Occlusion Compaction` / `Material Decal` spans are entirely absent from the both-off traces. Allocation footprint: `decal_color` (16 MB at 4K) + the lazy-grow HZB/occlusion/compaction/decal-classify buffers (sized via `ensure_capacity` only when their feature is on) all skip allocation when off. |
| 16.4.C | decal classify gates frustum + screen-AABB only | + HZB occlusion gate at decal screen-AABB footprint | Landed. WGSL template gains an `hzb_enabled` flag that conditionally emits the HZB binding + per-decal mip-selection gate. Only active when `features.gpu_culling && features.decals`. Per-frame cost is bounded by `MAX_DECAL_COUNT = 128` mip-selected textureLoads — sub-microsecond on the empty / synthetic tuning scenes (`Material Decal RenderPass` stays at 0.002-0.004 ms across both-on traces). Genuine perf win surfaces when authored scenes have decals occluded behind near-camera geometry; the synthetic decal-less tuning content can't exercise that path. |
| 16.7/16.8 | geometry pass records `draw_indexed` per mesh; cull + compaction output unconsumed | storage-array meta lookup + per-mesh `drawIndexedIndirect` consuming compaction's args buffer (gated behind `features.gpu_culling`) | Landed. Non-instanced meshes route through the new path; instanced meshes (`Mesh::instanced` — curve-instances, prefab-instances) keep the legacy uniform-with-dynamic-offset binding + `draw_indexed_with_instance_count`. Vertex shader template forks on `instancing_transforms`: non-instanced reads `geometry_mesh_metas[instance_index]` from a `var<storage, read> array<GeometryMeshMeta>` binding, populated at vertex entry into a `var<private>` so the shared helpers in `shared_wgsl/vertex/*.wgsl` keep compiling without parameter threading. The geometry pipeline now carries two layouts (`pipeline_layout_key_storage` + `pipeline_layout_key_uniform`); the shadow pass mirrors the split. CPU side: `render.rs` pre-uploads `IndirectDrawArgs.{index_count, first_instance = mesh_meta_idx}` per non-instanced opaque renderable each frame and populates `occlusion_instances[i].mesh_meta_offset` (was hard-zeroed); the compaction shader's `atomicAdd(instance_count)` then runs against a freshly-zeroed buffer each frame. **Measured** (post-§16.7/§16.8, vs pre-rewire numbers): tuning-1k-meshes 1.645 ms vs 2.043 ms (-0.398 ms, -19%); tuning-10k-meshes 1.912 ms vs 2.029 ms (-0.117 ms, -6%); legacy `features.gpu_culling = false` paths unchanged (visual parity confirmed by ~identical Render means across modes on both 1k and 10k). Shadow Generation similarly drops on the 10k tier (0.607 ms vs 0.744 ms pre-rewire) because the shadow VS shares the storage-array lookup, eliminating its per-mesh dynamic-offset `set_bind_group` call too. |
| T6   | provisional `OVERSIZED_LIST_COUNT_THRESHOLD = 16` / `OVERSIZED_AABB_DIAGONAL_METERS = 50.0` (`light_buckets/buckets.rs`) | re-tuned only if real content fails the defaults | Defaults validated against the existing tuning scenes via a new `read_oversized_mesh_stats` wasm helper (`#[cfg(debug_assertions)]`, debug builds only). `{last_max_bucket, oversized_count}` per scene: `tuning-open-world` 0/0 (directional sun only — no punctual buckets), `tuning-10k-meshes` 0/0 (same), `tuning-64-lights` 3/0 (small spheres don't trip the AABB threshold), `tuning-mixed-intensity` 66/1 (floor plane correctly flagged), `tuning-1k-meshes` 157/1 (floor plane correctly flagged, 157-deep bucket on the busiest light is acceptable since the floor is what's anchoring it). Net: the 16-mesh / 50 m defaults correctly identify the synthetic floor planes in dense scenes and don't false-positive on small meshes. Re-tune only when a real production scene with multiple terrain chunks + ocean planes shows a measurable false-negative; the harness is now ready for that re-run. |
| T3   | `score > 4.0 / > 1.0 / > 0.1` cutoffs in `shadows/importance.rs` (Ultra / High / Medium) | re-tuned to `> 10.0 / > 1.0 / > 0.05` | Landed. New cutoffs tuned against the new `tuning-importance-tiers` scene (16 lights in a 4×4 `distance × intensity` grid, distances ∈ {1, 5, 15, 50} m, intensities ∈ {1, 10, 100, 1000}). On that scene the old cutoffs produced 7 Low / 4 Medium / 1 High / 4 Ultra (the [1, 4] High band was too narrow to catch mid-bright + mid-distance lights); the new cutoffs give 6 / 5 / 3 / 2. Verified non-regression on the other tuning scenes: `tuning-mixed-intensity` 14 Low / 6 Medium (was 16 / 4), `tuning-1k-meshes` 10 / 10 (was 20 / 0), `tuning-64-lights` 10 / 0 (unchanged — uniform intensities don't span enough range). Drove via a new `read_importance_tier_histogram` `#[wasm_bindgen]` helper that recomputes the score per shadow-casting light + buckets against the cutoffs; mirrors the live values in `light_importance_decision`. Games tuning their own content should override per-light tiers explicitly via `LightShadowParams::tier`; these cutoffs are the *default* heuristic. |
| T4   | `Mesh::cheap_material_pixel_threshold: u32 = 64` hard-coded | `Option<u32>` per-mesh + `AwsmRenderer::default_cheap_material_pixel_threshold: u32 = 64` global | Landed. Mesh field is `Option<u32>` (`None` = use renderer default); `effective_material_key` takes an explicit `default_threshold` argument so the caller can pick from any source. Renderer-side default is a single field games can write directly each frame from whatever quality system they have (or leave at 64). No automatic coupling to `ShadowQualityTier` — that's per-light and threading "the global tier" through `collect_renderables` would lock in a cross-system dependency that's awkward to undo. Future-proof: the knob is small + composable; a game that wants tier-tied behavior writes `renderer.default_cheap_material_pixel_threshold = tier.threshold()` whenever its tier changes. |
