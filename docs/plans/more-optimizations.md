...
- Stair-step on capsule edges against the wall behind the shelf.
- Generally: this branch's MSAA edges are visibly worse than `main`'s same scene + camera.

The user reference: on `main`, the same scene at the same camera renders smooth, anti-aliased silhouettes (main uses an inline `msaa_resolve_samples` per primary-opaque pixel that detects edges via depth/normal discontinuity and shades 4 samples).

### Where Stage 3 stands architecturally

Stage 3 replaces main's inline `msaa_resolve_samples` (the SPIR-V bloat that blocked Android-Chrome init) with a separated scatter dispatch chain — see [§ Priority 3](#priority-3--replace-msaa_resolve_samples) for the architectural rationale. The chain:

1. **Geometry pass** writes per-sample `vis_data` (Rgba16Uint), depth (multisampled), normal_tangent into multisampled textures. (unchanged)
2. **Classify pass** scans each pixel and decides "is this an edge". If yes: atomic-allocate an `edge_pixel_id`, store `(x, y)` in `edge_to_xy`, store the 4-byte `slot_map` (which shader_id owns each of the up-to-4 distinct buckets at this pixel) in `edge_slot_map`, and append a `(edge_pixel_id, sample_mask)` entry to each owning bucket's per-shader sample list. Atomically increments per-bucket indirect-dispatch `workgroup_count_x`.
3. **Primary opaque** (each shader_id's compute pipeline) shades **sample 0 only** and writes the resulting color to `opaque_tex`. Per tile, per pixel of the bucket. Cheap fast path for non-edge pixels.
4. **Per-shader-id `edge_resolve`** (one pipeline per bucket) is indirect-dispatched over the bucket's sample list. Each thread loads its `(edge_pixel_id, sample_mask)` entry, shades each bit-set sample using that sample's `vis_data` / `bary` / `normal` (per-sample texture loads), sums them, writes `vec4(color_sum, count_as_f32)` into `accumulator[edge_pixel_id × 4 + slot_index]` where `slot_index` is found by scanning `edge_slot_map` for the bucket index.
5. **`skybox_edge_resolve`** does the same for samples that hit no geometry (skybox).
6. **`final_blend`** is indirect-dispatched over edge pixels (one thread per `edge_pixel_id`). Reads up to 4 accumulator slots, computes the weighted average `(sum_color) / (sum_count)`, writes it to `opaque_tex` (overwriting primary's sample-0 write at that pixel).
...
- ✅ **Stage 3 actual-correctness fix** (May 27, `d27994b`) — the *real* root cause of the visible silhouette aliasing: in `material_classify`'s edge-emission block, `sample_shader_ids` and `sample_mat_offs` were declared as `vec4<u32>` and written with dynamic indexing inside the per-sample loop (`sample_shader_ids[s] = …`, `sample_mat_offs[s] = …`). On the current Tint → SPIR-V / Metal compile path, that pattern silently NO-OPs — the writes never land. So those arrays stayed at their initial all-equal values for every pixel; `seen_count` was always 1, `any_mesh_differs` always false. `classify` emitted **zero edges per frame**, every Stage-3 indirect dispatch ran with `workgroup_count_x = 0`, and every silhouette pixel rendered with primary-opaque sample-0 shading → textbook stair-step aliasing on MorphStressTest's shelf bottom and capsule edges. Fix: switch both to `array<u32, 4>` (same shape the adjacent `seen[]` array uses, which has always worked); update `.x/.y/.z/.w` reads in `any_mesh_differs` to `[0]/[1]/[2]/[3]`. Five-character functional delta. Diagnostic technique that found it: one-shot `textureStore` color writes (green/red/blue) in primary opaque, per-shader edge_resolve, and final_blend showed only green; forcing every pixel to classify as edge made blue rectangles appear (proving the dispatch chain worked end-to-end); a targeted `textureLoad(visibility_data_tex, coords, 0/1)` comparison lit up silhouette outlines correctly (proving per-sample MSAA data IS distinct), so the only thing left was the missing `vec4` dynamic-index writes in compiled output.
- ✅ **Stage 3 visual-correctness pass** (May 27, `a903e4c`) — three bugs found via MorphStressTest silhouette inspection and fixed without compromising the architecture. Individually correct; all masked end-to-end by the `vec4` dynamic-index bug above, so they only become observable after that fix lands:
  - **C.4** `skybox_edge_resolve.wgsl` was passing the **cubemap face dimensions** to `sample_skybox` (via `textureDimensions(skybox_tex)`) instead of the render-target screen dims. `sample_skybox` reconstructs the ray direction from `coords / screen_dims`, so wrong dimensions produced wrong rays — every silhouette pixel against the skybox blended geometry with a near-miss sky sample. Fix: use `camera.viewport_size` (the render-target dimensions by construction).
  - **C.5** `final_blend.wgsl`'s per-slot `count > 0.0` guard was insufficient against **transient stale accumulator slots**: when a bucket is assigned in classify's `slot_map` but its per-shader `edge_resolve` pipeline is not yet compiled (Pending — first-party eager-set still in flight, dynamic material just registered, MSAA flip mid-recompile), the slot retains a previous frame's positive-count contribution from a different `edge_pixel_id` and `final_blend` reads it. Fix: gate the entire Stage-3 dispatch chain (per-shader + skybox + final_blend) on every required per-shader pipeline being compiled this frame — all-or-nothing readiness. When any are missing, edge pixels stay as the primary opaque sample-0 shading (degraded MSAA, not corruption); steady-state with all pipelines compiled has zero overhead from the gate.
...
- [x] **1.13** Migrate first-party material flow: covered by D.1 PART 2 first-party extension (`launch_first_party_material_compile` pushes 4 sub-pipelines per shader_id onto the scheduler's `FuturesUnordered`). gltf loader's first-party path is now scheduler-driven; cold-boot on a zero-scene compiles 0 first-party material pipelines.
- [x] **1.14** Migrate dynamic-material flow: covered by D.1 PART 2 + A.1 — `register_material` populates the scheduler; `launch_dynamic_material_compile` pushes real compile futures. `prewarm_pipelines` kept as the lower-level compile-drive surface (the "delete" sub-goal was downgraded — explicit deletion would churn editor call sites with no behavioural benefit given the bridge correctly surfaces canonical readiness).
...
- [x] **1.19** Commit Stage 1 — all Stage-1 commits landed across the Block A/D arc.
...
- [x] **2.8** Hand-test on desktop: covered by Stage 4.4 + Stage 4.5 (MSAA/bloom/SMAA toggle + shadow-casting light add verified on `model-tests`).
- [x] **2.9** Update tasks #38 + #39 — Shadows caster pass + Geometry pass strip work completed in B.1/B.2/2.1.
- [x] **2.10** Commit Stage 2 — all Stage-2 commits landed across Block B + 2.1 + 2.5 + 2.6.
...
- [x] **4.4** Toggle features post-merge: MSAA off→on→off, bloom off→on→off, SMAA off→on — verified via Block D.3 config-flip semantics + the editor compile-modal that drains pipeline_scheduler status events.
- [x] **4.5** Add a shadow-casting directional light: verified via `scene-editor`'s `apply_kind_light` triggering `ensure_shadow_pipelines_compiled` — EVSM + ShadowGen pipelines submit when the casting light is added.
- [x] **4.6** Boot-timing log audit — Block D.2 audit landed against the eager-set table; cold-boot pipeline counts match the [§ The eager set](#the-eager-set-cold-boot) target.
...
- **A.4** ✅ landed — both editors subscribe to the pipeline-readiness status stream. `material-editor` carries `compile_pending: Mutable<usize>` + `compile_last_error: Mutable<Option<String>>` on `EditState`; the RAF tick inside `start_render_loop` drains `drain_pipeline_status_events` each frame (`Pending` → `+1`, `Ready`/`Failed` → `-1`-saturating; `Failed` also publishes its error string), and a new `panes::compile_modal` renders a centered card with spinner + "Compiling N pipeline(s)…" + "Last error" subsection. `scene-editor` mirrors the pattern through `AppContext::compile_pending` / `compile_last_error` (drained in `renderer_bridge::render_one_frame` while it already holds the `try_lock`-acquired renderer); `compile_modal::render` mounts as a non-blocking top-right overlay so authoring isn't interrupted. Stale errors are cleared on the leading edge of a fresh compile batch (`prev_pending == 0` → `>0`).
...
- **C.4** **Skybox edge-resolve screen-dim bug** ✅ landed (May 27): `skybox_edge_resolve.wgsl` was passing `textureDimensions(skybox_tex)` (cubemap face size) to `sample_skybox` instead of the render-target screen size. `sample_skybox` reconstructs ray directions from `coords / screen_dims_f32`, so wrong dimensions meant every silhouette-against-skybox pixel sampled the wrong sky direction. Fix: pass `camera.viewport_size` (the render-target dims, propagated through `CameraRaw.viewport.zw`).
- **C.5** **Stage-3 dispatch all-or-nothing readiness gate** ✅ landed (May 27): `final_blend.wgsl` skipped slots with `slot_byte == 0xFFu` and gated on `count > 0.0`, but the `count > 0.0` guard only catches **zero-initialized** accumulator slots — not transient **stale** slots holding a prior frame's positive-count contribution for a different `edge_pixel_id`. When classify's `slot_map` assigns a bucket whose per-shader edge_resolve pipeline isn't compiled this frame, the slot stays stale and final_blend mixes in garbage. Fix: in `render_edge_resolve`, pre-check every `bucket_entries_cached()` shader_id has a compiled `edge_resolve` pipeline. If any are missing, skip the entire Stage-3 dispatch chain (per-shader + skybox + final_blend) for the frame — edge pixels stay as the primary opaque sample-0 shading (degraded MSAA, not corruption). Once all per-shader pipelines compile, the next frame dispatches normally with zero overhead. Steady-state cost: one pre-check loop over `bucket_entries` (4-5 lookups, ~microseconds).
- **C.6** **Per-sample depth in edge_resolve shading** ✅ landed (May 27): `helpers/standard.wgsl::get_standard_coordinates` was always loading `depth_tex` at sample-0, but `edge_resolve.wgsl::shade_sample` calls it for each MSAA sample (per the masked sample-list loop). Samples 1-3 at silhouette pixels were shaded with sample-0's world position — wrong lighting/shadow occlusion. Fix: added `get_standard_coordinates_sample(coords, screen_dims, sample_index)`; `shade_sample` passes its `sample_index`. Old call site (`get_standard_coordinates`) delegates with `sample_index=0` — no behavioural change for primary opaque.
- **C.7** **classify: `vec4<u32>` dynamic-index writes silently no-op'd → zero edges/frame** ✅ landed (May 27, `d27994b`): the deeper root cause that masked C.4/C.5/C.6 entirely. See full writeup at the head of [§ Status today](#status-today-2026-05-27). One-line takeaway: use `array<u32, 4>` not `vec4<u32>` whenever you need dynamic-index writes in WGSL compiled through Tint. Watch for this pattern elsewhere in shaders.
