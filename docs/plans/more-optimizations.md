...
- [x] **3.2** Both `msaa_resolve_samples` + `msaa_process_sample` **deleted** from `helpers/material_shading.wgsl`. Both call sites in `compute.wgsl` (skybox-overlap + edge-pixel branches) replaced with early-return. Primary opaque pipelines are fast-path only. **Known regression**: edge pixels render as transparent black until dispatch wiring (3.7-fully) lands.
