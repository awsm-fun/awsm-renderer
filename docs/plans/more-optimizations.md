...
- [~] **3.2** Both `msaa_resolve_samples` + `msaa_process_sample` were **deleted** from `helpers/material_shading.wgsl` by the agent (commit `6057906`). The deletion was then **temporarily reverted** at `58f0316` for desktop correctness while dispatch wiring landed. After Stage 3.7 dispatch wiring completed at `df23238`, the inline path is **conditionally active** — it stays on for devices that fall below the new path's bind-group cap (`maxBindGroups < 5`, e.g. macOS Metal); it deactivates and the new edge_resolve pipelines take over on Android Vulkan. The SPIR-V-shrinking goal is realized on Android (the target device for the original failure); macOS desktop pays no compile penalty because the inline path stays there.
...
- [x] **3.7** Pipeline + bind-group infrastructure complete. `MaterialEdgePipelines::ensure_compiled()` walks the bucket list + globals and runs them through Shaders::ensure_keys + ComputePipelines::ensure_keys. `RenderContext::material_edge_buffers` + `material_edge_layout_uniform` plumbing in place. Per-frame `reset_header` called from `AwsmRenderer::render()`. `render_edge_resolve` builds runtime bind groups + dispatches per-shader + skybox + final_blend pipelines. `emit_edge_data: true` flipped on classify cache key (gated on device-cap support). **Device-cap gate**: macOS Metal's `maxBindGroups=4` can't fit the per-shader edge_resolve layout (5 groups), so on devices that fall below the needed limits, `edge_resolve_supported(gpu)` returns false and the inline `msaa_resolve_samples` path stays active. On Android Vulkan (`maxBindGroups=8`) the new path activates. Follow-up: restructure layout to fit in 4 groups (fold edge buffer into extended primary opaque group(0); needs storage-buffer-count audit).
