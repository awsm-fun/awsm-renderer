...
- [~] **3.2** `msaa_resolve_samples` + `msaa_process_sample` were deleted then restored multiple times during the iterative dispatch wiring. Current state at HEAD: **inline path active** in `compute.wgsl` and `helpers/material_shading.wgsl` (restored at `828950a`) because the new path is force-disabled pending the args/data buffer split. When `edge_resolve_supported` is flipped to true after the split, the inline path comes back out in tandem with the new path activation.
...
- [~] **3.7** Pipeline + bind-group infrastructure complete and wired end-to-end (4-group layout via the shadows-group(3) fold from a later commit, dispatch wiring in `render_edge_resolve`, classify `emit_edge_data` flag flip). **However**: discovered a fundamental WebGPU validation conflict — `MaterialEdgeBuffers` is bound as Storage(read-write) AND used as Indirect for `dispatch_workgroups_indirect` in the same compute pass sync scope, which WebGPU rejects. Splitting per-pass doesn't help (validation is per-pass-buffer-usage). The fix requires splitting `MaterialEdgeBuffers` into two GpuBuffers — `args_buffer` (indirect-args + counters only) and `data_buffer` (storage). Pending that follow-up, `edge_resolve_supported()` force-returns false; the inline `msaa_resolve_samples` path stays active. All edge_resolve infrastructure (pipelines, bind groups, classify emit flag) remains in place but dormant.
