...
- [~] **3.2** Both `msaa_resolve_samples` + `msaa_process_sample` were **deleted** from `helpers/material_shading.wgsl` by the agent (commit `6057906`). The deletion was then **temporarily reverted** at the parent's `58f0316` so the renderer keeps producing correct edge pixels until the dispatch wiring (3.7-fully) lands. When dispatch wiring lands: re-revert `58f0316` to land the SPIR-V-shrinking deletion + flip `emit_edge_data: true` together.
...
- [~] **3.7** Pipeline + bind-group infrastructure (`MaterialEdgePipelines`, `MaterialEdgeBindGroupLayouts`) threaded through `describe_shaders` / `from_resolved` + onto `MaterialOpaqueRenderPass`. `render_edge_resolve` entry point exists and short-circuits via `warn_pipeline_not_compiled` until pipelines submitted. **Dispatch wiring is incomplete** — `RenderContext::material_edge_buffers` plumbing + bind-group construction + `dispatchWorkgroupsIndirect` calls + scheduler-submit of the new pipelines + `emit_edge_data: true` flag flip on classify cache key all remain TODO. Deferred to post-PR follow-up.
