...
- [ ] **1.16** Update `docs/dynamic-materials/contract-opaque.md` and `contract-transparent.md`: registration is non-blocking, status checked via `pipeline_group_status` or `subscribe_pipeline_status`, no `.await` on prewarm. (Today's contract docs don't yet reference `prewarm_pipelines` directly — they document the per-shading-pass contract surface; check whether updates are needed by greping for `prewarm` / `register_material` in those files before editing.)
...
- [ ] **1.18** Hand-test on desktop: `task material-editor:dev` + `task scene-editor:dev` + `task model-tests:dev`. Verify `phase = Ready` in <100 ms via boot-timing logs. Verify gltf load + incremental paint works (skybox visible immediately, mesh appears later). **Deferred until 1.8 fully + 1.13/1.14 land** — at this commit point, the existing eager-compile path still runs, so cold-boot behaviour is unchanged from main. The new scheduler is wired but not load-bearing yet.
