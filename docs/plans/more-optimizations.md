...
**Status**: Sprints 1–4 landed in
[PR](https://github.com/dakom/awsm-renderer/pull/103) on the
`more-optimizations` branch (May 2026). Outstanding work: Tier 4
architectural items only; T1.9 picker batch (deferred — API change);
T2.4 LoadOp::Load fix (architectural fuse); T2.7 mapAsync polling
(deferred — spawn_local is already idiomatic in wasm). See per-sprint
commit messages for the full landed/skipped breakdown.
...
## Part 5 — How to validate progress
