...
pub enum UvMode {
    StretchOnce,
    RepeatByLength { u_repeat: f32, v_repeat_per_unit: f32 },
}
...
pub fn sweep_along_curve<C: Curve3 + ?Sized>(curve: &C, cross_section: &CrossSection, opts: &SweepOpts) -> MeshData {
    ...
    for (i, frame) in frames.frames.iter().enumerate() {
        let v = match &opts.uv_mode {
            ...
            UvMode::RepeatByLength { v_repeat_per_unit, .. } => {
    ...
    let edge_count = if profile_closed { n_profile } else { n_profile - 1 };
...
fn cross_section_points(cs: &CrossSection) -> Vec<Vec2> {
    match cs {
        ...
        CrossSection::Tube { radius, radial_segments } => {
        ...
        CrossSection::Profile { points, .. } => points.iter().map(|p| Vec2::new(p[0], p[1])).collect(),
    }
}
...
mod tests {
    ...
    fn sweep_tube_has_expected_vertex_count() {
        ...
        let cs = CrossSection::Tube { radius: 0.5, radial_segments: 8 };
        let opts = SweepOpts { samples: 16, ..Default::default() };
