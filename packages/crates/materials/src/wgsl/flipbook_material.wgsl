...
//
// The cell-selection math (mode consts, flipbook_apply_mode,
// flipbook_is_past_end, flipbook_cell_uv) lives in `flipbook_cell.wgsl`,
// concatenated BEFORE this file in `WGSL_FRAGMENT` — see flipbook.rs.
//
...
fn flipbook_compute_cell_uv(material: FlipBookMaterial, in_uv: vec2<f32>, current_time: f32) -> vec2<f32> {
    // Delegates to the shared scalar cell math (flipbook_cell.wgsl) — the
    // SAME function the masked geometry/shadow variants call, so the cutout
    // can never disagree with the shaded cell.
    return flipbook_cell_uv(
        in_uv,
        current_time,
        material.cols,
        material.rows,
        material.frame_count,
        material.fps,
