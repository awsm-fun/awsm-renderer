...
fn brdf_ibl_with_transmission(
    ...
    if (effective_transmission > 0.0) {
        ...
        let irradiance = sampleIrradiance(n, ibl_irradiance_tex, ibl_irradiance_sampler, ibl_info.irr_rot);
    ...
    var transmit_back = vec3<f32>(0.0);
    {% if pbr_features.diffuse_transmission %}
    let back_irradiance = sampleIrradiance(-n, ibl_irradiance_tex, ibl_irradiance_sampler, ibl_info.irr_rot);
    ...
    let irradiance_sheen = sampleIrradiance(n, ibl_irradiance_tex, ibl_irradiance_sampler, ibl_info.irr_rot);
